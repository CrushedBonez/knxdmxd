...
bool json_get_trigger(struct json_object *trigger, const char trigger_type,
  ...
  case TRIGGER_STEPDIM:
    ...
    break;
  case TRIGGER_FADING:
    json_object_object_get_ex(trigger, "fadingga", &trigger_knx);
    json_object_object_get_ex(trigger, "value", &trigger_value);
    break;
  default:
    json_object_object_get_ex(trigger, "knx", &trigger_knx);
    json_object_object_get_ex(trigger, "value", &trigger_value);
    break;
  }
...
bool json_get_cue(cue_t *cue, struct json_object *json_cue, const int cuenum,
  ...
  json_object_object_get_ex(json_cue, "name", &name);
  strdcopy(&cue->name, (name) ? json_object_get_string(name) : "unknown");

// check link first
  struct json_object *link;
  json_object_object_get_ex(json_cue, "link", &link);
  ...
  struct json_object *json_channels;
  json_object_object_get_ex(json_cue,
      "data", &json_channels);
  if (!json_channels) {
    syslog(LOG_INFO,
  ...
  for (i = 0; i < cue->data_len; i++) { // read all
    ...
    struct json_object *channel = json_object_array_get_idx(json_channels,
        i);
    struct json_object *chan;
    json_object_object_get_ex(channel, "channel", &chan);
    struct json_object *value;
    json_object_object_get_ex(channel, "value", &value);
  ...
  struct json_object *fading;
  json_object_object_get_ex(json_cue, "fading", &fading);
  if (fading) {
    struct json_object *fading_time;
    json_object_object_get_ex(fading, "time", &fading_time);
    if (!fading_time) {
      struct json_object *fading_time_in;
      json_object_object_get_ex(fading, "in", &fading_time_in);
      struct json_object *fading_time_out;
      json_object_object_get_ex(fading,"out", &fading_time_out);
  ...
  struct json_object *waittime;
  json_object_object_get_ex(json_cue, "waittime", &waittime);
  if (waittime) {
  ...
  json_object_object_get_ex(json_cue, "delay", &delay);
  if (delay) {
...
void load_config() {
  ...
  json_object_object_get_ex(config, "channels", &in_data);
  ...
  for (i = 0; i < channel_num; i++) { // read all
    ...
    struct json_object *dmx;
    json_object_object_get_ex(element, "dmx", &dmx);
    struct json_object *statusga;
    json_object_object_get_ex(element, "statusga", &statusga);

    struct json_object *factor;
    json_object_object_get_ex(element, "factor", &factor);
    ...
    if (statusga) {
      if (json_object_get_type(statusga) == json_type_string) { // old style config, value only
        ...
        struct json_object *valuega;
	json_object_object_get_ex(statusga, "value", &valuega);
        channels[i].valuega =
            valuega ? str2knx(json_object_get_string(valuega)) : 0;
        struct json_object *switchga;
	json_object_object_get_ex(statusga, "switch", &switchga);
        channels[i].switchga =
            switchga ?
  ...
  json_object_object_get_ex(config, "dimmers", &in_data);
  ...
  for (i = 0; i < dimmer_num; i++) { // read all
    ...
    json_object_object_get_ex(element, "name", &name);
    struct json_object *dmx;
    json_object_object_get_ex(element, "channel", &dmx);
    struct json_object *ga;
    json_object_object_get_ex(element, "ga", &ga);
    ...
    if (json_object_get_type(ga) == json_type_string) { // old style config, value only
      ...
      struct json_object *valuega;
      json_object_object_get_ex(ga, "value", &valuega);
      struct json_object *switchga;
      json_object_object_get_ex(ga, "switch", &switchga);
      struct json_object *stepga;
      json_object_object_get_ex(ga, "dim", &stepga);
    ...
    // get turnonvalue
    struct json_object *tov;
    json_object_object_get_ex(element, "turnonvalue", &tov);
    if (tov) {
    ...
    // get fading
    struct json_object *fading;
    json_object_object_get_ex(element, "fading", &fading);
    dimmers[i].fading = fading ? json_object_get_double(fading) : 0.0;

    struct json_object *dimtime;
    json_object_object_get_ex(element, "dimtime", &dimtime);
    dimmers[i].dimtime = dimtime ? json_object_get_double(dimtime) : 0.0;

    struct json_object *fading_ga;
    json_object_object_get_ex(element, "fadingga", &fading_ga);
    if (fading_ga) {
  ...
  json_object_object_get_ex(config, "scenes", &in_data);
  ...
  for (i = 0; i < scene_num; i++) { // read all
    ...
    if (json_get_cue(&(scenes[i]), scene, i, TARGET_SCENE)) { // we still need the go-trigger
      struct json_object *triggers;
      json_object_object_get_ex(scene, "trigger", &triggers);
      if (!triggers) {
        syslog(LOG_INFO, "load_config: scene %s has no trigger defined",
            scenes[i].name);
        continue;
      }
      struct json_object *go;
      json_object_object_get_ex(triggers, "go", &go);
      ...
      json_object_object_get_ex(triggers, "dim", &dim);
  ...
  struct json_object *json_cuelists;
  json_object_object_get_ex(config, "cuelists", &json_cuelists);
  ...
  for (i = 0; i < cuelist_num; i++) { // read all
    ...
    struct json_object *name;
    json_object_object_get_ex(cuelist, "name", &name);
    strdcopy(&cuelists[i].name,
        (name) ? json_object_get_string(name) : "unknown");

    struct json_object *roh;
    json_object_object_get_ex(cuelist, "release_on_halt", &roh);
    cuelists[i].release_on_halt =
        roh ? json_object_get_boolean(roh) : false;

    struct json_object *pog;
    json_object_object_get_ex(cuelist, "proceed_on_go", &pog);
    cuelists[i].proceed_on_go = pog ? json_object_get_boolean(pog) : true;

    struct json_object *cues;
    json_object_object_get_ex(cuelist, "cues", &cues);
    if (!cues) {
      syslog(LOG_INFO,
    ...
    struct json_object *triggers;
    json_object_object_get_ex(cuelist, "trigger", &triggers);
    if (!triggers) {
      syslog(LOG_INFO,
    ...
    struct json_object *triggers_go;
    struct json_object *triggers_halt;
    struct json_object *triggers_direct;
    struct json_object *triggers_dim;
    struct json_object *triggers_release;
    json_object_object_get_ex(triggers, "go", &triggers_go);
    json_object_object_get_ex(triggers, "halt", &triggers_halt);
    json_object_object_get_ex(triggers, "direct", &triggers_direct);
    json_object_object_get_ex(triggers, "dim", &triggers_dim);
    json_object_object_get_ex(triggers, "release", &triggers_release);
    if (json_get_trigger(triggers_go, TRIGGER_GO,
    TARGET_CUELIST, i)) {
    ...
    if (json_get_trigger(triggers_halt,
    TRIGGER_HALT,
    TARGET_CUELIST, i)) {
      syslog(LOG_DEBUG,
    ...
    if (json_get_trigger(triggers_direct,
